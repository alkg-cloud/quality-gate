...
def test_passing_report_renders_success_header() -> None:
    report = {
        "gate_passed": True, "regressions": [], "warnings": [],
        "passing": ["coverage_global"], "bootstrap": False,
    }
    pr_metrics = {
        "coverage": {"lines_pct": 80.0, "files": {}}, "duplication": {"pct": 1.0},
...
def test_failing_report_lists_regressions() -> None:
    report = {
        "gate_passed": False,
        "regressions": [
            {"metric": "lint", "scope": "file", "file": "a.py", "baseline": 0, "actual": 5, "delta": 5},  # noqa: E501
        ],
    ...
    baseline = {
        "metrics": {**pr_metrics, "lint": {"total": 0, "by_file": {}}},
        "commit_sha": "a" * 40, "updated_at": "2026-05-26T00:00:00Z",
    }
    md = render_pr_comment(pr_metrics, baseline, report)
    assert "❌ FAILED" in md
    assert "a.py" in md
...
def test_bootstrap_report_explains_first_run() -> None:
    report = {
        "gate_passed": True, "regressions": [], "warnings": [], "passing": [], "bootstrap": True,
    }
    pr_metrics = {
        "coverage": {"lines_pct": 1.0, "files": {}}, "duplication": {"pct": 1.0},
        "lint": {"total": 0, "by_file": {}}, "file_size": {"max_lines": 300, "violations": {}},
        "security": {"critical": 0, "high": 0, "moderate": 0, "low": 0}, "_meta": _meta(),
    }
