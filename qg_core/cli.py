...
def _cmd_compare(args: argparse.Namespace) -> int:
    ...
    baseline = (
        None if args.baseline == "NONE"
        else json.loads(Path(args.baseline).read_text(encoding="utf-8"))
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    c = sub.add_parser("render-badges")
    c.add_argument("--metrics", required=True)
    c.add_argument("--output-dir", required=True)
    c.set_defaults(fn=_cmd_render_badges)

    c = sub.add_parser("baseline-payload")
    c.add_argument("--metrics", required=True)
    c.add_argument("--config", required=True)
    c.add_argument("--commit-sha", required=True)
    c.add_argument("--ref", required=True)
    c.add_argument("--output", required=True)
    c.set_defaults(fn=_cmd_baseline_payload)

    c = sub.add_parser("commit-message")
    c.add_argument("--short-sha", required=True)
    c.add_argument("--before", required=True)
    c.add_argument("--after", required=True)
    c.set_defaults(fn=_cmd_commit_message)

    c = sub.add_parser("exit-code")
    c.add_argument("--report", required=True)
    c.set_defaults(fn=_cmd_exit_code)
